...
A **company** is the organization record at the top of everything in Denowatts — the tenant. Solar sites belong to a company, every user account belongs to (at most) one company, and what you can see in the portal is decided by which company you're in. The company record also carries the organization's billing details, its API key for pulling data out programmatically, and the rules for who gets notified when something happens on its sites.

> **Reading this doc:** use the **Business / Developer** switch at the top. *Business* explains what a company is, what the record holds, and the access rules. *Developer* adds the full GraphQL surface, the services and tenant-scoping logic, every schema and DTO shape, file references, and a terminology primer.

---

## Why this matters

Companies are the walls between customers. Almost every other feature — users, sites, events, notifications, the public data API — leans on the company record to decide who may see or touch what. If a company's boundaries are wrong, one customer can see another's plants; if its notification settings are wrong, the right people don't hear about an alarm. It is the foundational record everything else attaches to.

---

## What a company record holds

- **Name and billing details** — address, city, state, ZIP, billing email and phone.
- **API key and IP whitelist** — the secret key external systems use to pull this company's data through the public data API, plus the list of IP addresses allowed to use it.
- **Site tags** — free-form labels the company uses to group and filter its sites.
- **Verified email domains** — email domains the company owns; a new signup whose email address matches one of these is attached to the company automatically when they confirm their account.
- **Notification settings** — one rule per alarm severity or event category, saying whether site managers and/or named extra users should receive an email and/or an in-app notification for that type.

---

## Companies as the tenant boundary

- **Users belong to a company.** Admins and regular users only ever see their own company's data; SuperAdmins (platform staff) see everything.
- **Sites are owned by a company**, and a site can additionally be *shared* with other companies — that's how cross-company access works.
- **The public data API authenticates by company**: a request must present the company's API key, come from a whitelisted IP address, and ask about a site the company owns or has been granted access to.
- **An internal "Denowatts" company** exists for the platform operator; it is always included in site-tag listings so internal sites stay visible.

---

## Who can do what

- **SuperAdmins only**: create companies, list and search all companies, edit any company in full, and generate or revoke API keys.
- **Any signed-in user**: view their own company's profile and its users, and edit the company's **site tags** — nothing else.
- **Notification settings** are edited on the Notification Management page, which saves through the same company-edit path (see the caveat below).

---

## The rules that matter

- **API keys are all-or-nothing.** Generating a key replaces any previous one; revoking removes it entirely. Either change takes effect on the public data API immediately.
- **The IP whitelist is an exact-match list** — individual addresses only, no ranges.
- **Non-admin edits are silently reduced to site tags.** When anyone other than a SuperAdmin saves a company, every field except site tags is dropped on the server. Known caveat: the Notification Management page is reachable by ordinary company users but saves through this same path, so notification-settings changes made by a non-admin may silently not persist. Whether this is intended is not yet confirmed — flagged for human review.
- **Site-tag lookups require a company.** A non-SuperAdmin who isn't attached to a company can't list site tags.
...
## Module wiring — `companies.module.ts` {dev}
...
## Data touched (collection `companies`) {dev}
...
- **Company (tenant)** — the top-level organization record; the multi-tenancy boundary every user, site, and notification rule attaches to.
- **Tenant scoping** — restricting queries/results to the caller's own company; for the `@AllRoles()` operations it is enforced in the service layer, not the guard.
- **API key (`apiKey`)** — the per-company secret (32-char dash-stripped UUIDv4) that authenticates the public data-out API. See [[data-out]].
- **IP whitelist (`whitelistedIPs`)** — exact-match list of IP addresses allowed to use the company's API key; no CIDR/range support.
- **Site tags (`siteTags`)** — free-form labels for grouping a company's sites; the only field non-SuperAdmins may edit via `updateCompany`.
- **Verified domains (`verifiedDomains`)** — email domains owned by the company; `denowatts.com` marks the internal Denowatts company.
- **Notification settings (`notificationSettings`)** — the per-type recipient rules consumed by both the alarm/webhook dispatcher and the events feed. See [[notification]] and [[events]].
- **Site access (`Site.accesses[]`)** — per-site grants letting other companies see a site; expanded via `getSitesAccessCompanies`. See [[site]].
- **Denowatts internal company** — the platform operator's own company, always merged into site-tag results so internal sites stay visible.
- **SuperAdmin (`SUPER_ADMIN`)** — platform staff role that bypasses role checks and company scoping. See [[users]].
